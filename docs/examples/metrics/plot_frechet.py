...
This example illustrates discrete Frechet Distance computation between time 
...
The optimal path, that is the path that minimizes the maximum cost to go from
the first time point to the last one, is represented in white on the image.

This computation method is similar to Dynamic Time Warping (DTW) but instead of
summing the costs, it takes the maximum cost of the optimal path.
...
path, pair, sim = metrics.frechet_path_pair(s_y1, s_y2)
